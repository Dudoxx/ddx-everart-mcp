...
function errorResponse(error: EverArtError): {
  ...
  return {
    content: [
      ...
      ...(error.details
        ? [{ type: 'text', text: `Details: ${JSON.stringify(error.details, null, 2)}` }]
        : []),
    ],
    isError: true,
  };
...
async function processWebProjectPath(
  basePath?: string,
  projectType?: string,
  assetPath?: string
...
async function saveImage(
  ...
  if (!validateModelFormatCompatibility(model, format)) {
    throw new Error(
      `Format '${format}' is not compatible with model '${model}'. SVG format is only available with Recraft-Vector (8000) model.`
    );
  }
  ...
  try {
    ...
    if (format === 'svg') {
      ...
      const result = optimize(svgString, {
        multipass: true,
        plugins: ['preset-default', 'removeDimensions', 'removeViewBox', 'cleanupIds'],
      });
...
function handleApiError(error: unknown): EverArtError {
  const err = error as {
    response?: {
      status: number;
      statusText: string;
  ...
  switch (request.params.name) {
    case 'generate_image': {
      try {
        ...
        const format =
          (args.format as string | undefined) || (modelInput === '8000' ? 'svg' : 'png');
